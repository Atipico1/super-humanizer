...
선정한 사람 글 **100편**을 대상으로 모델별 글쓰기 습관을 분석하는 프로젝트입니다. 현재 **34편**과 같은 소재로 Opus 5·Fable 5.1·Astra 6가 쓴 **102편**의 비교를 마쳤으며, 나머지 선정 글로 분석을 확대할 계획입니다. 어떤 표현이 나왔는지뿐 아니라, 사람 글보다 얼마나 자주 쓰였는지, 어느 모델과 문맥에서 두드러지는지 살펴 윤문 지침을 구성합니다.
...
초기 10편과 추가 24편에서 반복 확인했고, 요약 유래 내용을 제외해도 사람 대비 밀도 차이가 유지된 핵심 관찰은 다음과 같습니다. 해당 모델만의 독점적 특징을 뜻하지는 않습니다.

| 작성 모델 | 이번 표본에서 반복 확인한 특징 |
|---|---|
| Opus 5 | 대조를 통한 재정의가 조밀하고, 짧은 독립 문단으로 판단·전환을 강조 |
| Fable 5.1 | Opus와 함께 대조를 통한 재정의가 사람 글보다 조밀함 |
| Astra 6 | 유보한 뒤 제한된 긍정으로 돌아오는 구성의 밀도가 세 모델 중 가장 높음 |

**[모델별 확인 결과와 수치](docs/MODEL_FINDINGS.md)**에서 정의, 초기·추가 표본 비교, 엠대시·따옴표 실측과 확정 목록에서 제외한 항목을 볼 수 있습니다. [집계 JSON](docs/model-findings-data.json)도 공개합니다. 스킬의 조건부 점검 예시가 모두 확정된 모델 특성을 뜻하지는 않습니다.

표본 구성과 절차는 [연구 요약](docs/RESEARCH.md)에 있습니다. 최신 지침이 경쟁 스킬보다 우수하다거나 모델별 지침의 추가 효과가 입증됐다는 주장은 하지 않습니다.
...
docs/
├── RESEARCH.md                      # 공개 연구 요약
├── MODEL_FINDINGS.md                # 반복 확인한 모델별 특징
└── model-findings-data.json          # 핵심 문체·부호 집계
